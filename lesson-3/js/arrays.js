...
output.textContent = `The first element in my array is ${myArray[2]}.`;
// You can also change a particular element
myArray[1] = false;
output.textContent = `The second element in my array is ${myArray[1]}`;
...
output.textContent = `The second element within the fourth element in my array is ${myArray[3] [1]}.`;
...
output.textContent = `There are ${myArray[3].length} items in myArray array.`;
output.textContent = "";
...
for (let i = 0; i < myArray.length; i ++) {
    ...
    if (Array.isArray(myArray[i])) {
        ...
        // We have an array1 So lets build a loop
